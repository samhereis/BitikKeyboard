...
fun SymbolKeyboardView(
    onKeyPress: (String) -> Unit, isShiftEnabled: androidx.compose.runtime.MutableState<Boolean>
) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(KeyboardStyle.rowSpacingDp)) {
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(KeyboardStyle.keySpacingDp)) {
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(KeyboardStyle.keySpacingDp)) {
            ...
            row3.forEach { key ->
                KeyButton(
                    ...
                    isSystem = false,
                    width = keyWidth,
                    backgroundColorIndex = (if (isShiftEnabled.value) key.backgroundColorIndexUppercase else key.backgroundColorIndexUppercase) ?: 1,
                    previewOnTap = true,
                    isShiftEnabled = isShiftEnabled,
                    onKeyPress = { onKeyPress(if (isShiftEnabled.value) key.uppercase else key.lowercase) },
                    onLongPress = {
